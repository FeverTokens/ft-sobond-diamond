...
import {closeEvents, collectEvents, getEvents} from "../tests/events";
...
describe("Run tests of the Coupon process", function () {
	...
	async function init(): Promise<void> {
		...
		const primary = await allContracts
			...
			.deploy(bnd.newi({maxGas: 1000000}), register.deployedAt, 100 * 10_000);
	...
	describe("Coupon proces", function () {
		...
		it("should pass the initialization and check the status", async () => {
			...
			const totalSupply = Number.parseInt(
				await register.totalSupply(cak.call()),
			);
			const bndBalance = Number.parseInt(
				await register.balanceOf(cak.call(), await bnd.account()),
			);
		...
		it("should fail to deploy the coupon smart contract when the deployer has not the PAY role", async () => {
			...
			).to.be.rejectedWith("Sender must be a Paying calculation agent");
		...
		it("should deploy the coupon and get paymentID for an investor (max 16 hexa chars excluding 0x)", async () => {
			...
			const concatenated = web3.utils.encodePacked(
				coupon.deployedAt,
				investorAddress1,
			); //abi.encodePacked
			const expectedPaymentId = web3.utils
				.keccak256(concatenated ?? "")
				.substring(0, 18);

			const actual = (await coupon.paymentIdForInvest(
				payer.call(),
				investorAddress1,
			)) as string;
			expect(actual).to.equal(expectedPaymentId);
			expect(actual.substring(2).length).to.equal(
				16,
				"paymentId must be maximum 16 chars for Swift",
			);
		});
		...
		it("should enable the paying calculation agent to activate the coupon", async () => {
			...
			const actualCurrentTS = parseInt(
				await register.currentSnapshotDatetime(payer.call()),
			);
			expect(actualCurrentTS).to.equal(
				expectedCurrentTS,
				"1671290200  couponDate + cutofftime expected",
			);
		});
		...
		it("should enable the paying calculation agent to validate the coupon by calling setDateAsCurrentCoupon so that coupon status is set to Ready", async () => {
			...
			expect(await coupon.status(payer.call())).to.equal(
				"1",
				"Coupon ready status expected after coupon validation",
			);
		});
		...
		it("should calculate the payment amount for an investor", async () => {
			...
			let couponPaymentForInvestor = await coupon.getPaymentAmountForInvestor(
				payer.call(),
				await investorA.account(),
			);
		...
		it("should calculate the payment amount for several investors ", async () => {
			...
			const coupon2 = await allContracts
				.get(CouponTradeContractName)
				.deploy(
					payer.newi({maxGas: 2000000}),
					register.deployedAt,
			...
			let payment = Math.floor(
				(unitValue * couponRate * bal * nbDaysInPeriod) / 360,
			);

			//then the coupon calculates the payment amount for investor
			let couponPaymentForInvestor2 = await coupon2.getPaymentAmountForInvestor(
				payer.call(),
				await investorB.account(),
			);

			expect(couponPaymentForInvestor2).to.equal(
				payment.toString(),
				"couponPaymentForInvestor2 not properly calculated",
			);
			...
			//test of the coupon
			const coupon3 = await allContracts
				.get(CouponTradeContractName)
				.deploy(
					payer.newi({maxGas: 2000000}),
					register.deployedAt,
			...
			bal = await register.balanceOfCoupon(
				cak.call(),
				await investorC.account(),
				firstCouponDate,
			);
			...
			//then the coupon calculates the payment amount for investor
			let couponPaymentForInvestor3 = await coupon3.getPaymentAmountForInvestor(
				payer.call(),
				await investorC.account(),
			);

			expect(couponPaymentForInvestor3).to.equal(
				payment.toString(),
				"couponPaymentForInvestor3 incorrectly calculated",
			);
			...
			const trade4 = await allContracts
				.get(BilateralTradeContractName)
				.deploy(
					bnd.newi({maxGas: 2000000}),
					register.deployedAt,
			...
			//test of the coupon
			const coupon4 = await allContracts
				.get(CouponTradeContractName)
				.deploy(
					payer.newi({maxGas: 2000000}),
					register.deployedAt,
			...
			//then the coupon calculates the payment amount for investor
			let couponPaymentForInvestor4 = await coupon4.getPaymentAmountForInvestor(
				payer.call(),
				await investorD.account(),
			);
			...
			const investorsAtCoupon: string[] =
				await register.getInvestorListAtCoupon(payer.call(), couponDate);
			// console.log("List of investors at coupon",couponDate, investorsAtCoupon);
			expect(investorsAtCoupon).include(await bnd.account());
			const balancesAtCoupon = (
				await Promise.all(
					investorsAtCoupon.map((inv) =>
						register.balanceOfCoupon(payer.call(), inv, couponDate),
					),
				)
			).map((bal, i) => ({
				address: investorsAtCoupon[i],
				bal: Number.parseInt(bal),
			}));
			// console.log("Balances at coupon", balancesAtCoupon);
			const totalBalance = balancesAtCoupon.reduce(
		...
		it("should allow creating a coupon smart contract after the cutoff time", async () => {
			...
			await expect(
				coupon2.setDateAsCurrentCoupon(payer.send({maxGas: 300000})),
		...
		it("should deploy a second coupon and take snapshot of evolving balances", async () => {
			...
			console.log(
				"_currentCouponDate : " +
					(await register.currentCouponDate(cak.call())),
			);
			//console.log("------------------------------------------");
			...
			let unitValue = await register.getBondUnitValue(cak.call());
			let couponRate = await register.getBondCouponRate(cak.call());
			let balA = await register.balanceOfCoupon(
				cak.call(),
				await investorA.account(),
				firstCouponDate,
			);

			let paymentA = Math.floor(
				(unitValue * couponRate * balA * nbDaysInPeriod) / 360,
			);
			...
			let couponPaymentForInvestorA = await coupon.getPaymentAmountForInvestor(
				payer.call(),
				await investorA.account(),
			);
			...
			let balB = await register.balanceOfCoupon(
				cak.call(),
				await investorB.account(),
				firstCouponDate,
			);
			let paymentB = Math.floor(
				(unitValue * couponRate * balB * nbDaysInPeriod) / 360,
			);
			...
			let couponPaymentForInvestorB = await coupon.getPaymentAmountForInvestor(
				payer.call(),
				await investorB.account(),
			);
			...
			//------------------------------------------------------------------------------------------------------------------
			console.log(
				"block number before : " + (await web3.eth.getBlockNumber()),
			...
			console.log(
				"block number after  : " + (await web3.eth.getBlockNumber()),
				await blockTimestamp(),
			);
			//------------------------------------------------------------------------------------------------------------------
			//console.log("------------------------------------------");
			...
			let balAbefore = await register.balanceOf(
				cak.call(),
				await investorA.account(),
			);
			let balBbefore = await register.balanceOf(
				cak.call(),
				await investorB.account(),
			);
			...
			console.log(
				"balance Investor A after transferFrom : " +
					(await register.balanceOf(cak.call(), await investorA.account())),
			);

			console.log("balance Investor B before transferFrom : " + balBbefore);
			console.log(
				"balance Investor B after transferFrom : " +
			...
			balA = await register.balanceOfCoupon(
				cak.call(),
				await investorA.account(),
				couponDate,
			);
			...
			couponPaymentForInvestorA = await coupon2.getPaymentAmountForInvestor(
				payer.call(),
				await investorA.account(),
			);
			...
			expect(couponPaymentForInvestorA).to.equal(
				paymentA.toString(),
				"Invalid payment A",
			);

			//We calculate the payment amount for the second coupon for investorB
			balB = await register.balanceOfCoupon(
				cak.call(),
				await investorB.account(),
				couponDate,
			);
			...
			couponPaymentForInvestorB = await coupon2.getPaymentAmountForInvestor(
				payer.call(),
				await investorB.account(),
			);
			...
			let balanceB = register.balanceOfCoupon(
				cak.call(),
				await investorB.account(),
				couponDate,
			);
			...
			expect(await register.currentSnapshotDatetime(payer.call())).to.equal(
				`${makeDateTime(recordDate, cutOffTimeInSec)}`,
			);
		});
		...
		it("should deploy a third coupon, take snapshot of evolving balances for 3 investors", async () => {
			...
			const coupon = await allContracts
				.get(CouponTradeContractName)
				.deploy(
					payer.newi({maxGas: maxGasAmount}),
					register.deployedAt,
					couponDate,
					nbDaysInPeriod,
					recordDate,
			...
			let balC = await register.balanceOfCoupon(
				cak.call(),
				await investorC.account(),
				firstCouponDate,
			);
			let paymentC = Math.floor(
				(unitValue * couponRate * balC * nbDaysInPeriod) / 360,
			);
			...
			let couponPaymentForInvestorC = await coupon.getPaymentAmountForInvestor(
				payer.call(),
				await investorC.account(),
			);
			...
			let balOfCoupon1Before = [
				await register.balanceOfCoupon(
					cak.call(),
					await investorA.account(),
					firstCouponDate,
				),
			...
			let gas = await register.transferFrom(
				cak.test(),
				await investorA.account(),
				await investorB.account(),
				12,
			...
			balC = await register.balanceOfCoupon(
				cak.call(),
				await investorC.account(),
				couponDate,
			);
			...
			couponPaymentForInvestorC = await coupon2.getPaymentAmountForInvestor(
				payer.call(),
				await investorC.account(),
			);
			...
			let balanceC = register.balanceOfCoupon(
				cak.call(),
				await investorC.account(),
				couponDate,
			);
			...
			//check balances before transfer 1
			let balAbeforetransfer3 = await register.balanceOf(
				cak.call(),
				await investorA.account(),
			);
			let balBbeforetransfer3 = await register.balanceOf(
				cak.call(),
				await investorB.account(),
			);
			let balCbeforetransfer3 = await register.balanceOf(
				cak.call(),
				await investorC.account(),
			);
			...
			//check balances before transfer 2
			let balAbeforetransfer4 = await register.balanceOf(
				cak.call(),
				await investorA.account(),
			);
			let balBbeforetransfer4 = await register.balanceOf(
				cak.call(),
				await investorB.account(),
			);
			let balCbeforetransfer4 = await register.balanceOf(
				cak.call(),
				await investorC.account(),
			);
			...
			couponPaymentForInvestorA = await coupon3.getPaymentAmountForInvestor(
				payer.call(),
				await investorA.account(),
			);
			...
			couponPaymentForInvestorB = await coupon3.getPaymentAmountForInvestor(
				payer.call(),
				await investorB.account(),
			);
			...
			balanceB = register.balanceOfCoupon(
				cak.call(),
				await investorB.account(),
				couponDate,
			);
			...
			couponPaymentForInvestorC = await coupon3.getPaymentAmountForInvestor(
				payer.call(),
				await investorC.account(),
			);
			...
			balanceC = register.balanceOfCoupon(
				cak.call(),
				await investorC.account(),
				couponDate,
			);
		...
		it("should deploy a third coupon, take snapshot of evolving balances for 3 investors make payment Ready, close coupon and finalize it", async () => {
			...
			let couponPaymentForInvestorA = await coupon4.getPaymentAmountForInvestor(
				payer.call(),
				await investorA.account(),
			);
			...
			await coupon4.toggleCouponPayment(
				cak.send({maxGas: 100000}),
				await investorA.account(),
			);
			...
			let couponPaymentForInvestorB = await coupon4.getPaymentAmountForInvestor(
				payer.call(),
				await investorB.account(),
			);
			...
			let couponPaymentForInvestorC = await coupon4.getPaymentAmountForInvestor(
				payer.call(),
				await investorC.account(),
			);
			...
			const coupon5 = await allContracts
				.get(CouponTradeContractName)
				.deploy(
					payer.newi({maxGas: maxGasAmount}),
					register.deployedAt,
			...
			//check balances before transfer 1
			let balAbeforetransfer1 = await register.balanceOf(
				cak.call(),
				await investorA.account(),
			);
			let balBbeforetransfer1 = await register.balanceOf(
				cak.call(),
				await investorB.account(),
			);
			let balCbeforetransfer1 = await register.balanceOf(
				cak.call(),
				await investorC.account(),
			);

			//do a transfer 1 so the balance moves
			gas = await register.transferFrom(
				cak.test(),
				await investorA.account(),
				await investorB.account(),
				12,
			...
			//check balances before transfer 2
			let balAbeforetransfer2 = await register.balanceOf(
				cak.call(),
				await investorA.account(),
			);
			let balBbeforetransfer2 = await register.balanceOf(
				cak.call(),
				await investorB.account(),
			);
			let balCbeforetransfer2 = await register.balanceOf(
				cak.call(),
				await investorC.account(),
			);
			...
			couponPaymentForInvestorA = await coupon5.getPaymentAmountForInvestor(
				payer.call(),
				await investorA.account(),
			);
			...
			await coupon5.toggleCouponPayment(
				cak.send({maxGas: 100000}),
				await investorA.account(),
			);
			...
			couponPaymentForInvestorB = await coupon5.getPaymentAmountForInvestor(
				payer.call(),
				await investorB.account(),
			);
			...
			couponPaymentForInvestorC = await coupon5.getPaymentAmountForInvestor(
				payer.call(),
				await investorC.account(),
			);
			...
			const coupon6 = await allContracts
				.get(CouponTradeContractName)
				.deploy(
					payer.newi({maxGas: maxGasAmount}),
					register.deployedAt,
			...
			couponPaymentForInvestorA = await coupon6.getPaymentAmountForInvestor(
				payer.call(),
				await investorA.account(),
			);
			...
			await coupon6.toggleCouponPayment(
				cak.send({maxGas: 100000}),
				await investorA.account(),
			);
			...
			couponPaymentForInvestorB = await coupon6.getPaymentAmountForInvestor(
				payer.call(),
				await investorB.account(),
			);
			...
			couponPaymentForInvestorC = await coupon6.getPaymentAmountForInvestor(
				payer.call(),
				await investorC.account(),
			);
