...
describe("Run tests on Register (Bond Issuance) contract", function () {
	...
	describe("Register contract: verify mint rules", function () {
		...
		it("CAK may mint", async function () {
			// console.log({cakAddress});
			const test = await instance.getBondData(cak.call());
			console.log("Testing Register Diamond: ", test);
			console.log("It's' working");
		});

		it("enableInvestorToWhitelist is working", async function () {
			console.log("yippee-ki-yay 0");

			const custodian = new Web3FunctionProvider(web3.currentProvider, (list) =>
				Promise.resolve(list[1]),
			);

			console.log("yippee-ki-yay 1");
			...
			console.log("yippee-ki-yay 2");

			const gas = await instance.enableInvestorToWhitelist(
				custodian.test(),
				strangerAddress,
			);
			...
			await instance.enableInvestorToWhitelist(
				custodian.send({maxGas: gas}),
				strangerAddress,
			);

			console.log("yippee-ki-yay 3");

			const isInvestorAllowed = await instance.investorsAllowed(
				cak.call(),
				strangerAddress,
			);

			console.log("isInvestorAllowed: ", isInvestorAllowed);
		});
	});
});
