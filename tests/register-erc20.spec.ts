...
describe("Register (Bond Issuance) contract interoperability", function () {
	...
	describe("with erc-20", function () {
		...
		it("allowance before mint is zero", async () => {
			const actual = await instance.allowance(
				cak.call(),
				instance.deployedAt,
				cakAddress,
			);
		...
		it("approve() should be denied", async () => {
			await expect(
				instance.approve(cak.send({maxGas: 130000}), strangerAddress, 30),
			).to.be.rejectedWith("approve is disabled");
		...
		it("transfer() should be denied", async () => {
			await expect(
				instance.transfer(cak.send({maxGas: 130000}), strangerAddress, 1),
